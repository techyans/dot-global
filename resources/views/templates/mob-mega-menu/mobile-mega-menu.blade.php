<div class="row py-4">
    <div class="col-12">
        <div class="logo">
            <a href="{{url('/')}}"><img src="{{asset('storage/images/20231110-191450-980554-dot-logo.png')}}" alt=""></a>
        </div>
    </div>
    <div class="col-12">
        <ul class="mob-first">
            <li class="first-li"><a href="javascript:void(0);">What We Do <i class="fa-solid fa-angle-down"></i></a>
                <ul class="first-ul">
                    <li class="mt-3">
                        <div class="main-list">
                            <p class="m-0 fw-bold">BMC</p>
                            <p class="m-0">Lorem Ipsum is simply dummy text of the printing.</p>
                        </div>
                        <ul class="second-list">
                            <li><a class="dropdown-item" href="#">BMC Helix ITSM</a></li>
                            <li><a class="dropdown-item" href="#">BMC Helix Remedyforce</a></li>
                            <li><a class="dropdown-item" href="#">BMC Helix Discovery</a></li>
                            <li><a class="dropdown-item" href="#">BMC TrueSight</a></li>
                            <li><a class="dropdown-item" href="#">BMC Control-M</a></li>
                            <li><a class="dropdown-item" href="#">BMC Smart Reporting</a></li>
                        </ul>
                    </li>
                    <li class="">
                        <div class="main-list">
                            <p class="m-0 fw-bold">Oracle</p>
                            <p class="m-0">Lorem Ipsum is simply dummy text of the printing.</p>
                        </div>
                        <ul class="second-list">
                            <li><a class="dropdown-item" href="#">Implementation</a></li>
                            <li><a class="dropdown-item" href="#">Managing ERP</a></li>
                            <li><a class="dropdown-item" href="#">Managing CRM</a></li>
                            <li><a class="dropdown-item" href="#">Supply Chain Operations</a></li>
                        </ul>
                    </li>
                    <li>
                        <div class="main-list">
                            <p class="m-0 fw-bold">Salesforce</p>
                            <p class="m-0">Lorem Ipsum is simply dummy text of the printing.</p>
                        </div>
                        <ul class="ps-3 second-list">
                            <li><a class="dropdown-item" href="#">Salesforce Sales Cloud</a></li>
                            <li><a class="dropdown-item" href="#">Salesforce Service Cloud</a></li>
                            <li><a class="dropdown-item" href="#">Salesforce Commerce Cloud</a></li>
                            <li><a class="dropdown-item" href="#">Salesforce Marketing Cloud</a></li>
                            <li><a class="dropdown-item" href="#">Salesforce Community Cloud</a></li>
                        </ul>
                    </li>
                    <li>
                        <div class="main-list">
                            <p class="m-0 fw-bold">Service Now</p>
                            <p class="m-0">Lorem Ipsum is simply dummy text of the printing.</p>
                        </div>
                        <ul class="ps-3 second-list">
                            <li><a class="dropdown-item" href="#">IT Service Management</a></li>
                            <li><a class="dropdown-item" href="#">IT Workflows</a></li>
                            <li><a class="dropdown-item" href="#">Audit Management</a></li>
                            <li><a class="dropdown-item" href="#">Configuration Compliance</a></li>
                        </ul>
                    </li>
                    <li>
                        <div class="main-list">
                            <p class="m-0 fw-bold">AWS</p>
                            <p class="m-0">Lorem Ipsum is simply dummy text of the printing.</p>
                        </div>
                        <ul class="ps-3 second-list">
                            <li><a class="dropdown-item" href="#">AWS Cloud Migration</a></li>
                            <li><a class="dropdown-item" href="#">AWS Managed Cloud Services</a></li>
                            <li><a class="dropdown-item" href="#">AWS Consulting</a></li>
                        </ul>
                    </li>
                    <li>
                        <div class="main-list">
                            <p class="m-0 fw-bold">SAP</p>
                            <p class="m-0">Lorem Ipsum is simply dummy text of the printing.</p>
                        </div>
                        <ul class="ps-3 second-list">
                            <li><a class="dropdown-item" href="#">SAP CRM</a></li>
                            <li><a class="dropdown-item" href="#">SAP HANA</a></li>
                            <li><a class="dropdown-item" href="#">SAP ERP</a></li>
                            <li><a class="dropdown-item" href="#">SAP PLM</a></li>
                            <li><a class="dropdown-item" href="#">SAP HR</a></li>
                        </ul>
                    </li>
                    <li>
                        <div class="main-list">
                            <p class="m-0 fw-bold">Dynamics 365</p>
                            <p class="m-0">Lorem Ipsum is simply dummy text of the printing.</p>
                        </div>
                        <ul class="ps-3 second-list">
                            <li><a class="dropdown-item" href="#">BMC Helix ITSM s</a></li>
                            <li><a class="dropdown-item" href="#">BMC Helix Remedyforce</a></li>
                            <li><a class="dropdown-item" href="#">BMC Helix Discovery</a></li>
                            <li><a class="dropdown-item" href="#">BMC TrueSight</a></li>
                            <li><a class="dropdown-item" href="#">BMC Control-M</a></li>
                            <li><a class="dropdown-item" href="#">BMC Smart Reporting</a></li>
                            <li><a class="dropdown-item" href="#">BMC Client Management</a></li>
                            <li><a class="dropdown-item" href="#">BMC Helix Digital Work Place</a></li>
                            <li><a class="dropdown-item" href="#">BMC BladeLogic</a></li>
                            <li><a class="dropdown-item" href="#">BMC Training</a></li>
                        </ul>
                    </li>
                    <li>
                        <div class="main-list">
                            <p class="m-0 fw-bold">Google Analytics</p>
                            <p class="m-0">Lorem Ipsum is simply dummy text of the printing.</p>
                        </div>
                        <ul class="ps-3 second-list">
                            <li><a class="dropdown-item" href="#">Power BI</a></li>
                            <li><a class="dropdown-item" href="#">QlikView</a></li>
                            <li><a class="dropdown-item" href="#">Qlik Sense</a></li>
                            <li><a class="dropdown-item" href="#">Tableau</a></li>
                            <li><a class="dropdown-item" href="#">SAP Analytics</a></li>
                        </ul>
                    </li>
                </ul>
            </li>

            <li class="first-li"><a href="javascript:void(0);">Offering <i class="fa-solid fa-angle-down"></i></a>
                <ul class="first-ul">
                    <li class="mt-3">
                        <div class="main-list">
                            <p class="m-0 fw-bold">Solutions</p>
                            <p class="m-0">We build on the IT domain expertise and industry knowledge to design sustainable technology solutions.</p>
                        </div>
                        <ul class="second-list">
                            <li><a class="dropdown-item" href="#">ERP</a></li>
                            <li><a class="dropdown-item" href="#">CRM</a></li>
                            <li><a class="dropdown-item" href="#">Operations Management</a></li>
                            <li><a class="dropdown-item" href="#">Financial Management</a></li>
                            <li><a class="dropdown-item" href="#">Payment Management</a></li>
                            <li><a class="dropdown-item" href="#">Supply Chain Management</a></li>
                            <li><a class="dropdown-item" href="#">Asset Management</a></li>
                            <li><a class="dropdown-item" href="#">Fleet Management</a></li>
                            <li><a class="dropdown-item" href="#">Data Analytics</a></li>
                            <li><a class="dropdown-item" href="#">Human Resources</a></li>
                            <li><a class="dropdown-item" href="#">Document Management</a></li>
                            <li><a class="dropdown-item" href="#">eLearning</a></li>
                            <li><a class="dropdown-item" href="#">Project Management</a></li>
                            <li><a class="dropdown-item" href="#">Ecommerce</a></li>
                            <li><a class="dropdown-item" href="#">Marketing & Advertising</a></li>
                            <li><a class="dropdown-item" href="#">Content Management</a></li>
                            <li><a class="dropdown-item" href="#">Kiosk Software</a></li>
                            <li><a class="dropdown-item" href="#">Web Portals</a></li>
                        </ul>
                    </li>
                    <li class="mt-3">
                        <div class="main-list">
                            <p class="m-0 fw-bold">Services</p>
                            <p class="m-0">Our service portfolio covers an entire software development life cycle and meets varied business needs.</p>
                        </div>
                        <ul class="second-list">
                            <li><a class="dropdown-item" href="#">Software Development</a></li>
                            <li><a class="dropdown-item" href="#">Web Development</a></li>
                            <li><a class="dropdown-item" href="#">Mobile App Development</a></li>
                            <li><a class="dropdown-item" href="#">Testing and QA</a></li>
                            <li><a class="dropdown-item" href="#">Application Services</a></li>
                            <li><a class="dropdown-item" href="#">UI/UX Design</a></li>
                            <li><a class="dropdown-item" href="#">Infrastructure Services</a></li>
                            <li><a class="dropdown-item" href="#">Digital Transformation</a></li>
                            <li><a class="dropdown-item" href="#">Managed IT Services</a></li>
                            <li><a class="dropdown-item" href="#">IT Outsourcing</a></li>
                            <li><a class="dropdown-item" href="#">IT Consulting</a></li>
                            <li><a class="dropdown-item" href="#">IT Support</a></li>
                            <li><a class="dropdown-item" href="#">Data Analytics</a></li>
                            <li><a class="dropdown-item" href="#">Cybersecurity</a></li>
                        </ul>
                    </li>
                </ul>
            </li>

            <li class="first-li"><a href="javascript:void(0);">Innovation <i class="fa-solid fa-angle-down"></i></a>
                <ul class="first-ul">
                    <li class="mt-3">
                        <div class="main-list">
                            <p class="m-0 fw-bold">Technologies</p>
                            <p class="m-0">Our expertise spans all major technologies and platforms, and advances to innovative technology trends.</p>
                        </div>
                        <ul class="second-list">
                            <li><a class="dropdown-item" href="#">Cloud</a></li>
                            <li><a class="dropdown-item" href="#">Software as a Service</a></li>
                            <li><a class="dropdown-item" href="#">Internet of Things</a></li>
                            <li><a class="dropdown-item" href="#">Big Data</a></li>
                            <li><a class="dropdown-item" href="#">Data science</a></li>
                            <li><a class="dropdown-item" href="#">Machine learning</a></li>
                            <li><a class="dropdown-item" href="#">Artificial Intelligence</a></li>
                            <li><a class="dropdown-item" href="#">Blockchain</a></li>
                            <li><a class="dropdown-item" href="#">Virtual Reality</a></li>
                            <li><a class="dropdown-item" href="#">Augmented Reality</a></li>
                            <li><a class="dropdown-item" href="#">Computer Vision</a></li>
                            <li><a class="dropdown-item" href="#">Microsoft</a></li>
                            <li><a class="dropdown-item" href="#">Azure</a></li>
                            <li><a class="dropdown-item" href="#">Power Apps</a></li>
                            <li><a class="dropdown-item" href="#">Dynamics 365</a></li>
                            <li><a class="dropdown-item" href="#">Power BI</a></li>
                            <li><a class="dropdown-item" href="#">Amazon Web Services</a></li>
                            <li><a class="dropdown-item" href="#">Adobe Commerce</a></li>
                            <li><a class="dropdown-item" href="#">ServiceNow®</a></li>
                            <li><a class="dropdown-item" href="#">Salesforce</a></li>
                            <li><a class="dropdown-item" href="#">Pimcore</a></li>
                        </ul>
                    </li>
                    <li class="mt-3">
                        <div class="main-list">
                            <p class="m-0 fw-bold">Industries</p>
                            <p class="m-0">Our service portfolio covers an entire software development life cycle and meets varied business needs.</p>
                        </div>
                        <ul class="second-list">
                            <li><a class="dropdown-item" href="#">Financial Services</a></li>
                            <li><a class="dropdown-item" href="#">Banking</a></li>
                            <li><a class="dropdown-item" href="#">Insurance</a></li>
                            <li><a class="dropdown-item" href="#">Lending</a></li>
                            <li><a class="dropdown-item" href="#">Investment</a></li>
                            <li><a class="dropdown-item" href="#">FinTech</a></li>
                            <li><a class="dropdown-item" href="#">Payments</a></li>
                            <li><a class="dropdown-item" href="#">Retail</a></li>
                            <li><a class="dropdown-item" href="#">Healthcare</a></li>
                            <li><a class="dropdown-item" href="#">Manufacturing</a></li>
                            <li><a class="dropdown-item" href="#">Professional Services</a></li>
                            <li><a class="dropdown-item" href="#">Transportation and Logistics</a></li>
                            <li><a class="dropdown-item" href="#">Telecommunications</a></li>
                            <li><a class="dropdown-item" href="#">Oil and Gas</a></li>
                            <li><a class="dropdown-item" href="#">Construction</a></li>
                            <li><a class="dropdown-item" href="#">Travel and Hospitality</a></li>
                            <li><a class="dropdown-item" href="#">BMC BladeLogic</a></li>
                        </ul>
                    </li>
                </ul>
            </li>
            <li class="first-li"><a href="javascript:void(0);">Staff Augmentation <i class="fa-solid fa-angle-down"></i>
            </li>
            <li class="first-li"><a href="javascript:void(0);">Company <i class="fa-solid fa-angle-down"></i></a>
                <ul class="first-ul company-ul pt-3 ps-2">
                    <li><a class="dropdown-item" href="#">About Us</a></li>
                    <li><a class="dropdown-item" href="#">Testimonials</a></li>
                    <li><a class="dropdown-item" href="#">Contact Us</a></li>
                    <li><a class="dropdown-item" href="#">Company Broucher</a></li>
                    <li><a class="dropdown-item" href="#">Job Opening</a></li>
                    <li><a class="dropdown-item" href="#">C++</a></li>
                    <li><a class="dropdown-item" href="#">Free Trail</a></li>
                    <li><a class="dropdown-item" href="#">Portfolio</a></li>
                </ul>
            </li>
        </ul>
    </div>
</div>