<div class="row justify-content-center">
    <div class="col-12 col-md-8 col-lg-5 text-center">
        <h2>Trusted Clientele</h2>
        <p>DGT Offers services, including implementation, customization, integration, upgradation, and migrations.</p>
    </div>
</div>
<div class="row justify-content-center brands">
    <div class="col-12 col-lg-11 slides mx-auto">
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231231-193257-872790-Al_Rajhi_Bank_Logo-min.png')}}" alt="" class="w-100">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-083215-290980-AXA-min.png')}}" alt="" class="w-100">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-083215-710984-BACS.jpeg')}}" alt="" class="w-100">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-083215-168522-MIS-min.png')}}" alt="" class="w-100">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-083215-256372-Saudi-Aramco-logo-min.png')}}" alt="" class="w-100">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-084749-971638-Tawuniya-min-removebg-preview.png')}}" alt="" class="w-100 p-3">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-083215-468831-SAIP-min.png')}}" alt="" class="w-100 p-3">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-083215-877891-UniGlass.jpeg')}}" alt="" class="w-100 p-3">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-083215-424126-Urban Hardware-min.png')}}" alt="" class="w-100 p-3">
            </div>
        </div>
        <div class="text-center">
            <div class="content d-flex align-items-center mx-auto">
                <img src="{{asset('storage/images/20231228-083215-857784-wiz-motors-ltd-min.png')}}" alt="" class="w-100 p-3">
            </div>
        </div>
        {{-- <div class="row slides justify-content-center">
            <div class="text-center">
                <div class="content d-flex align-items-center">
                    <img src="{{asset('storage/images/20231210-185757-263907-any-desk.png')}}" alt="" class="w-100">
                </div>
            </div>
            <div class="text-center">
                <div class="content d-flex align-items-center">
                    <img src="{{asset('storage/images/20231210-185757-358438-aramko.png')}}" alt="" class="w-100">
                </div>
            </div>
            <div class="text-center">
                <div class="content d-flex align-items-center">
                    <img src="{{asset('storage/images/20231210-185757-739181-delloite.png')}}" alt="" class="w-100">
                </div>
            </div>
            <div class="text-center">
                <div class="content d-flex align-items-center">
                    <img src="{{asset('storage/images/20231210-185757-337981-ibm.png')}}" alt="" class="w-100">
                </div>
            </div>
            <div class="text-center">
                <div class="content d-flex align-items-center">
                    <img src="{{asset('storage/images/20231210-185757-189322-Tieto-evry.png')}}" alt="" class="w-100 p-3">
                </div>
            </div>
        </div> --}}
    </div>
</div>